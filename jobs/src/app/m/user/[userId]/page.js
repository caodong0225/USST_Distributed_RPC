            ...
            <div className="w-full h-full bg-[rgba(100,100,100,0.5)] p-10 pb-15 z-10 flex items-center flex-wrap backdrop-blur-lg">
              ...
              <div className="ml-10">
                <div className="text-2xl font-bold flex items-center gap-2 text-white">
                  ...
                  {userData?.role != null ?
                    userData.role == "admin"
                      ? <Tag color="red" key={'admin'}>管理员</Tag>
                      : <Tag color="orange" key={'super-admin'}>超级管理员</Tag>
                    : <Tag color="blue" key={'guest'}>普通用户</Tag>}
                </div>
                <div className="text-gray-200">{userData?.user?.email}</div>
                <div className="text-gray-300 mt-1.5">
                  <span>注册于 </span>
                  <span>{timestampToDateTime(userData?.createdAt)}</span>
                </div>
              </div>
            </div>
